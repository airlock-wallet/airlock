...
class NativeZetaChainSigner {
    ...
    /**
     * Smart amount parsing: supports 0x hexadecimal and decimal strings
     * @private
     */
    _parseAmount(val) {
        if (!val) return "0";
     ...
     * @param {string} txData.chainId - [Required] Chain ID (e.g., "athens_7001-1")
     ...
     * @param {string} txData.fromAddress - [Required] Sender address (zeta1...)
     * @param {string} txData.toAddress - [Required] Recipient address (zeta1...)
     * @param {string|number} txData.amount - [Required] Transfer amount (decimal or 0x)
     * @param {string} [txData.denom="azeta"] - Currency unit
     * @param {number|string} [txData.gas=200000] - Gas Limit
     * @param {string} [txData.memo=""] - Memo
     * @param coinType - Coin type
     * @returns {SignerResult}
     */
    signTransfer(txData, coinType = this.core.CoinType.nativeZetaChain) {
        ...
        const input = Cosmos.SigningInput.create({
            ...
            // ZetaChain specific hasher
            txHasher: Cosmos.TxHasher.Keccak256,
            fee: Cosmos.Fee.create({
                gas: this._toLong(txData.gas || 200000)
            }),
            // Must configure corresponding Ethermint public key type
            signerInfo: Cosmos.SignerInfo.create({
        ...
        if (inputError) throw new Error(`ZetaChain Input Error: ${inputError}`);
        ...
        if (outputVerifyError) throw new Error(`ZetaChain Output Error: ${outputVerifyError}`);

        if (output.errorMessage || output.error) {
            throw new Error(`ZetaChain Logic Error: ${output.errorMessage || output.error}`);
        }
...
module.exports = NativeZetaChainSigner;
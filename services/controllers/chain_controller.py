...
async def api_get_balance(chain_key: str, address: str, contract: Optional[str] = Query(None)):
...
async def get_account_resource(chain_key: str, address: str, contract: Optional[str] = Query(None)):
    return await blockchain_service.get_account_resource(chain_key, address, contract)

@router.get("/transaction/{chain_key}/{address}")
async def api_get_transactions(chain_key: str,  address: str,  contract: Optional[str] = Query(None),  limit: int = Query(10)):
...
async def api_get_utxo(chain_key: str, address: str, total_value: str = Query('0.00000001')):
    return await blockchain_service.get_utxo(chain_key, address, total_value)

@router.get("/block/{chain_key}/{address}")
async def api_get_block(chain_key: str, address: str):
...
async def api_get_utxo(chain_key: str):
    return await blockchain_service.get_fee(chain_key)

@router.get("/nonce/{chain_key}/{address}")
async def api_get_utxo(chain_key: str, address: str):
...
async def api_get_tx(chain_key: str, tx_id: str):
...
async def api_broadcast(chain_key: str, tx_hex: str = Body(..., embed=True)):
